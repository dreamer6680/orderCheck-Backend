...
The backend migration directory intentionally contains **one consolidated migration**:
`src/main/resources/db/migration/V9__current_schema.sql`.

- **New, empty PostgreSQL databases:** Flyway runs the single V9 migration and creates the final schema.
...
- **Databases already upgraded to V8:** the original V1–V8 files have been
  ...
  application ignores *missing* historical migrations and runs idempotent V9 while retaining
...
The consolidated V9 does **not** replace incremental migrations for future
schema changes. New changes must receive a new version (V10, V11, etc.);
